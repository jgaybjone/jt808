    ...
    public Publisher<Message> process(Message m) {
        return Mono.justOrEmpty(Optional.ofNullable(m)
                ...
                .map(message -> {
                    ...
                    gpsRecordService.saveEntity(fetch(h, b));
                    // todo 确认报警待开发
                    return CommonMessageUtils.success(h);
                }));
    }

    public static GpsRecord fetch(Header h, LocationAndAlarmMsg b) {
        final GpsRecord.GpsRecordBuilder gpsRecordBuilder = GpsRecord
                .builder()
                .time(Timestamp.valueOf(b.getTime()))
                .simNo(h.getSimNo())
                .longitude(BigDecimal.valueOf(b.getLongitude()).divide(BigDecimal.valueOf(1000000)).doubleValue())
                .latitude(BigDecimal.valueOf(b.getLatitude()).divide(BigDecimal.valueOf(1000000)).doubleValue())
